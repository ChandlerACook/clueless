...
public class GameBoard {
    ...
    private Deck deck;
    private Map<Player, Boolean> playerAccusationFailed;
    ...
    public GameBoard(List<Player> players) {
        ...
        this.playerAccusationFailed = new HashMap<>();
    ...
    public Card performDisproval(Player suggestingPlayer, Card suspect, Card weapon, Card room) {
        ...
        for (int i = 0; i < players.size() - 1; i++) {
            ...
            if (!matchingCards.isEmpty()) {
                ...
                Card cardToShow = matchingCards.get(0);

                // Show the card to the suggesting player
    ...
    public void performSuggestion(Player player, Card suspect, Card weapon, Card room) {
        if (player.getCurrentRoom() == null) {
            JOptionPane.showMessageDialog(null, "Player's current room is not set.");
            return;
        }
    ...
    public void performAccusation(Player player, Card suspect, Card weapon, Card room, Deck deck) {
        ...
        if (isCorrect) {
            // Correct accusation logic (end the game, announce winner)
            JOptionPane.showMessageDialog(null, player.getName() + " has made the correct accusation! You win!",
                    "Game Over", JOptionPane.INFORMATION_MESSAGE);
            endGame(player); // End the game and declare the winner
        } else {
            // Incorrect accusation logic (penalize or eliminate player)
            JOptionPane.showMessageDialog(null, player.getName() + " made an incorrect accusation. They are eliminated!",
                    "Accusation Result", JOptionPane.INFORMATION_MESSAGE);
            eliminatePlayer(player); // Eliminate the player from the game
        }
    }
    ...
    private void displayAccusation(Player player, Card suspect, Card weapon, Card room, boolean isCorrect) {
        ...
        if (isCorrect) {
            ...
            message = player.getName() + " made an incorrect accusation.";
        }
    }
